...
def counter_generated_data(datas):
  ...
  if len(excel_txt5) > 0:
    ...
    for i in range(0,len(umumiy_without_duplicate[2])):
      if (umumiy_without_duplicate[9][i] =='' or umumiy_without_duplicate[9][i] =='nan'):
        new_d[0]+=['001' for j in range(0,6)]
        new_d[1]+=['KLAES_REST' for j in range(0,3)]
        new_d[1]+=['KLAES_PP' for j in range(0,3)]
        new_d[2]+=['MARA' for j in range(0,6)]
        new_d[3]+=[ umumiy_without_duplicate[2][i] for j in range(0,6)]
        new_d[4]+=['KLAES','KLS_LENGTH','CH_PROFILE_TYPE','KLS_WAST','KLS_WAST_LENGTH','CH_KLAES_OPTM']
        new_d[5]+=[umumiy_without_duplicate[4][i],excel_txt5[i],umumiy_without_duplicate[5][i],umumiy_without_duplicate[6][i],umumiy_without_duplicate[7][i],umumiy_without_duplicate[8][i]]
      else:
