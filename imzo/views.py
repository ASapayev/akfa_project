...
def texcartaupload(request):
    ...
    for i in range(0,df.shape[0]):
        ...
        ekstruziya ='%.2f' % float(ekstruziya) ,
        pila ='%.2f' % float(pila),
        strayenie ='%.2f' % float(strayenie),
        skm_pokras = '%.2f' % float(skm_pokras),
        sat_pokras ='%.2f' % float(sat_pokras),
        gr_pokras = '%.2f' % float(gr_pokras),
        skm_xim = '%.2f' % float(skm_xim),
        sat_xim = '%.2f' % float(sat_xim),
        gr_xim = '%.2f' % float(gr_xim),
        ruchnoy_pokraska ='%.2f' % float(ruchnoy_pokraska),
        sublimat ='%.2f' % float(sublimat),
        nakleyka = '%.2f' % float(nakleyka),
        kombinirovan = '%.2f' % float(kombinirovan),
        upakovka = '%.2f' % float(upakovka),
        lam_nak_upakovka ='%.2f' % float(lam_nak_upakovka ),
        kom_upakovka = '%.2f' % float(kom_upakovka)
            ).save()
