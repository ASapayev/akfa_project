...
import pandas as pd
from .utils import get_norma_df,get_norma_price
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for key,row in df.iterrows():
                Siryo(
                    data ={'sap_code':row['SAPCODE'],'narx':row['NARX']}
                ).save()
            ...
            for i in range(1,25):
                if type_profile =='-LA/LC':
                    if df.iloc[key + i][names[0]] != '0' and '-LA' not in df.iloc[key + i][names[0]] and '-LC' not in df.iloc[key + i][names[0]] and '-7' not in df.iloc[key + i][names[0]] and df.iloc[key + i][names[1]] != '0':
                        ...
                        components.append([component_name,component_value,component_menge,'0',component_ves,'0',component_fakt])
                        # components.append([component_name,component_value,component_menge,'0'])
...
def generate_sap_code_price(sapcodes):
    ...
    for siryo in siryolar:
        siryo_dict[f'{siryo.data["sap_code"]}']=str(siryo.data["narx"])
    for i in range(0,len(sapcodes)):
        component_count = 0
        value = 0
        for j in range(0,len(sapcodes[i]['components'])):
            if sapcodes[i]['components'][j][0] in siryo_dict:
                sapcodes_copy[i]['components'][j][3] ="%.2f" % (float(siryo_dict[sapcodes[i]['components'][j][0]]) )
                sapcodes_copy[i]['components'][j][5] ="%.2f" % (float(siryo_dict[sapcodes[i]['components'][j][0]]) * float(sapcodes_copy[i]['components'][j][4]))
                value += float(siryo_dict[sapcodes[i]['components'][j][0]])
                component_count += 1
        if component_count == len(sapcodes[i]['components']):
            sapcodes_copy[i]['ves_corredted'] = True
            if Siryo.objects.filter(data__sap_code =sapcodes[i]['sap_code']):
                continue
            else:
                Siryo(
                    data ={'sap_code':sapcodes[i]['sap_code'],'narx':value}
                ).save()
    
    # print(sapcodes,'#'*20,sapcodes_copy)
