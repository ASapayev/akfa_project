            ...
            if countt:
                  ...
                  for i in range(0,len(sap_codes_list)):
                        if AluminiyProduct.objects.filter(artikul__icontains = sap_codes_list[i],kratkiy_tekst_materiala__icontains =kratkiy_texts[i]).exists():
                              obichniy = AluminiyProduct.objects.filter(artikul= sap_codes_list[i],kratkiy_tekst_materiala =kratkiy_texts[i])
                        ...
                        if AluminiyProductTermo.objects.filter(artikul__icontains = sap_codes_list[i],kratkiy_tekst_materiala__icontains =kratkiy_texts[i]).exists():
                              termo = AluminiyProductTermo.objects.filter(artikul__icontains = sap_codes_list[i],kratkiy_tekst_materiala=kratkiy_texts[i])
                              for ter in termo:
      ...
      if request.method =='POST':
            ...
            if sap_codes:
                  ...
                  for sap_code in sap_codes:
                        ...
                        if LengthOfProfile.objects.filter(Q(artikul =artikul)|Q(component =artikul)).exists():
                             
                              leng_of_profile_txt = LengthOfProfile.objects.filter(Q(artikul =artikul)|Q(component =artikul)).order_by('-created_at')[:1].get()
                              if leng_of_profile_txt.ves_za_metr[character_txt.surface_treatment]=='':
                                    all_correct=False
                        ...
                        if all_correct:
                              ...
                              if termo_component:
                                    length_profile = float(str(leng_of_profile_txt.ves_za_metr[character_txt.surface_treatment]).replace(',','.'))*(float(character_txt.length)/1000)
                                    character_dict['Общий вес за штуку'].append(length_profile)
                              else:
                                    ves_za_shtuk_ = float(str(leng_of_profile_txt.ves_za_metr[character_txt.surface_treatment]).replace(',','.'))*float(character_txt.length)/1000
                                    length_profile = ves_za_shtuk_
                                    character_dict['Общий вес за штуку'].append(ves_za_shtuk_)
...
def product_add_second_org(request,id):
      ...
      print(' boshlanish vaqti >> ',datetime.now().minute,datetime.now().second)
      ...
      order_id = request.GET.get('order_id',None)
      print(' togriligini tekshirish vaqti boshlash >> ',datetime.now().minute,datetime.now().second)
      if not correct:
            context ={
      ...
      print(' togriligini tekshirish vaqti tugashi>> ',datetime.now().minute,datetime.now().second)
      ################### group by#########
      ...
      for key,row in df.iterrows():
            ...
            if artikul !='nan':
                  ...
                  if ((row['Тип покрытия'] == 'Окрашенный') or (row['Тип покрытия'] == 'Белый' )):
                        ...
                        # print(df_new['Анодировка'][key],'<<<>>>>',row['Код цвета анодировки снаружи'],)
                        if row['Код наклейки'] != 'NT1':
                              # print(row['Контактность анодировки'],'nakkk')
      ...
      print('df_char vaqti boshlanishi>> ',datetime.now().minute,datetime.now().second)
      df_char = create_characteristika(cache_for_cratkiy_text) 
      print('df_char vaqti tugashi>> ',datetime.now().minute,datetime.now().second)
      print('df_char_title vaqti boshlanishi>> ',datetime.now().minute,datetime.now().second)
      df_char_title =create_characteristika_utils(cache_for_cratkiy_text)
      print('df_char_title vaqti tugashi>> ',datetime.now().minute,datetime.now().second)
      ...
      print('razlovka vaqti boshlanishi>> ',datetime.now().minute,datetime.now().second)
      razlovka_yoq = True
      ...
      for key, row in df_char_title.iterrows():
            if LengthOfProfile.objects.filter((Q(artikul=row['ch_article'])|Q(component=row['ch_article']))).exists():
                  ...
                  if length_of_profile.ves_za_metr[row['Тип покрытия']]!='':
                        ves_za_shtuk = float(length_of_profile.ves_za_metr[row['Тип покрытия']])*float(row['Длина'])/1000
                        df_char_title['Общий вес за штуку'][key] =ves_za_shtuk
                  ...
                  if LengthOfProfile.objects.filter(Q(artikul=row['ch_article'])|Q(component=row['ch_article'])).exists():
                        ...
                        if length_of_profile.ves_za_metr[row['Тип покрытия']]!='':
                              df_char_title['Общий вес за штуку'][key] =float(length_of_profile.ves_za_metr[row['Тип покрытия']].replace(',','.')) * float(row['Длина'].replace(',','.'))/(1000)
                              df_char_title['Удельный вес за метр'][key] = length_of_profile.ves_za_metr[row['Тип покрытия']]
                              price = Price.objects.filter(tip_pokritiya = row['Тип покрытия'],tip=row['ch_combination'])[:1].get()
                        ...
                        profile_doesnot_exists[1].append(baza_profile.data['Артикул'])
      ...
      print(' text yaratusg vaqti boshlanishi>> ',datetime.now().minute,datetime.now().second)
      if price_all_correct  and  work_type != 5:
            print(' text  update_char_title_function vaqti boshlanishi>> ',datetime.now().minute,datetime.now().second)
            path = update_char_title_function(df_char_title,df_extrusion,order_id,'aluminiytermo')
            print(' text  update_char_title_function vaqti tugashi>> ',datetime.now().minute,datetime.now().second)
