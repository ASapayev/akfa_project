...
def aluminiy_productbases(request):
  df = pd.read_excel('c:\\OpenServer\\domains\\База_Обичный.xlsx','Лист1')
#   print(df.shape)
#   print(df['Материал'][0])
#   print(df['Материал'][39705])
  
  for i in range(0,39706):
...
def alu_product_base(request):
  ...
  # print(df['Материал'][42],df['Материал'][0])
  return JsonResponse({'converted':'a'})
...
def product_add(request,id):
      ...
      for key,row in df.iterrows():
            ...
            dlina =''
            if row['Место для комментариев'] != 'nan':
                  if '.0' in row['Место для комментариев']:
                        dlina = row['Место для комментариев'].replace('.0','')
                  else:
                        dlina = row['Место для комментариев']
            ...
            if ((row['Тип покрытия'] == 'Окрашенный') or (row['Тип покрытия'] == 'Белый' )):
                  ...
                  if row['Код наклейки'] != 'NT1':
                        df_new['nkrat'][key] = df_new['pkrat'][key] +' '+ row['Код наклейки']
                  
                        
            elif row['Тип покрытия'] == 'Ламинированный':
                  df_new['pkrat'][key] = row['Сплав'][len(row['Сплав'])-2:] + row['тип закаленности'] +' L'+dlina+' '+row['Бренд краски снаружи']+row['Код краски снаружи']
                  if row['Код лам пленки снаружи'] != 'nan':
                        laminatsiya =row['Код лам пленки снаружи']+'/XXXX'
                  if row['Код лам пленки внутри'] != 'nan':
                        laminatsiya =+'XXXX/'+row['Код лам пленки внутри']
                        
                  df_new['lkrat'][key]=row['Сплав'][len(row['Сплав'])-2:] + row['тип закаленности'] +' L'+dlina+' '+row['Бренд краски снаружи']+row['Код краски снаружи']+'_'+laminatsiya
                  
                  if row['Код наклейки'] != 'NT1':
                        df_new['nkrat'][key]=df_new['lkrat'][key] + ' ' +row['Код наклейки']
                        
            elif row['Тип покрытия'] =='Сублимированный':
                  df_new['pkrat'][key]=row['Сплав'][len(row['Сплав'])-2:] + row['тип закаленности'] +' L'+dlina+' '+row['Бренд краски снаружи']+row['Код краски снаружи']
                  df_new['skrat'][key]=row['Сплав'][len(row['Сплав'])-2:] + row['тип закаленности'] +' L'+dlina+' '+row['Бренд краски снаружи']+row['Код краски снаружи']+'_'+row['Код декор пленки снаружи']
                  
                  if row['Код наклейки'] != 'NT1':
                        df_new['nkrat'][key]=df_new['skrat'][key] + ' ' + row['Код наклейки']

            elif row['Тип покрытия'] =='Анодированный':
                  df_new['akrat'][key]=row['Сплав'][len(row['Сплав'])-2:] +'T4 '+' L'+dlina+' '+row['Код цвета анодировки снаружи'].replace('.0','')
                  if row['Код наклейки'] != 'NT1':
                        df_new['nkrat'][key]= df_new['akrat'][key] +' ' + row['Код наклейки']
            else:
            ...
            if row['Место для комментариев'] != 'nan':
                  if AluminiyProduct.objects.filter(artikul =component,section ='F',kratkiy_tekst_materiala=df_new['fkrat'][key]).exists():
                        df_new['fkrat_counter'][key] = AluminiyProduct.objects.filter(artikul = component,section ='F',kratkiy_tekst_materiala=df_new['fkrat'][key])[:1].get().material
                  else: 
                        if AluminiyProduct.objects.filter(artikul =component,section ='F').exists():
                              umumiy_counter[component+'-F'] += 1
                              max_valuesF = umumiy_counter[ component +'-F']
                              materiale = component +"-F{:03d}".format(max_valuesF)
                              AluminiyProduct(artikul =component,section ='F',counter=max_valuesF,gruppa_materialov='ALUPF',kombinirovanniy='БЕЗ ТЕРМОМОСТА',kratkiy_tekst_materiala=df_new['fkrat'][key],material=materiale).save()
                              df_new['fkrat_counter'][key]=materiale
                        else:
                              materiale = component +"-F{:03d}".format(1)
                              AluminiyProduct(artikul =component,section ='F',counter=1,gruppa_materialov='ALUPF',kombinirovanniy='БЕЗ ТЕРМОМОСТА',kratkiy_tekst_materiala=df_new['fkrat'][key],material=materiale).save()
                              df_new['fkrat_counter'][key]=materiale
                  ...
                  if AluminiyProduct.objects.filter(artikul =row['Артикул'],section ='75',kratkiy_tekst_materiala=df_new['ukrat2'][key]).exists():
                        ...
                        if AluminiyProduct.objects.filter(artikul =component,section ='75').exists():
                              umumiy_counter[component+'-75'] += 1
                              max_values75 = umumiy_counter[ component +'-75']
                  ...
                  if AluminiyProduct.objects.filter(artikul =row['Артикул'],section ='7',kratkiy_tekst_materiala=df_new['ukrat1'][key]).exists():
                        ...
                        if AluminiyProduct.objects.filter(artikul =component,section ='7').exists():
                              umumiy_counter[component+'-7'] += 1
                              max_values7 = umumiy_counter[ component +'-7']
