...
def save_razlovka(df_new,file_type):
    if file_type =='simple':
        for key,razlov in df_new.iterrows():
            if not RazlovkaObichniy.objects.filter((Q(sap_code7=razlov['SAP код 7'])&Q(kratkiy7=razlov['U-Упаковка + Готовая Продукция'])&~Q(sap_code7='')&~Q(sap_code7='nan'))|(Q(sap_code75=razlov['SAP код 75'])&Q(kratkiy75=razlov['U-Упаковка + Готовая Продукция 75']))).exists():
                    RazlovkaObichniy(
