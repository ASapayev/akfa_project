...
class OrderSaveView(APIView):
    ...
    def post(self, request):
        ...
        for key,val in response.items():
            ...
            if 'acs' not in order_type and 'kraska' not in order_type and 'akp' not in order_type and 'prochiye' not in order_type and 'change' not in order_type and 'bussines' not in order_type and 'epdm' not in order_type:
...
def imzo_artikul_list(request):
    ...
    if term:
        artikules = AluProfilesData.objects.filter((Q(data__Артикул__icontains = term)|Q(data__Компонент__icontains=term))&(~Q(data__Артикул__exact = '0'))&(~Q(data__Артикул__exact = 0))&(~Q(data__Артикул__exact = ' '))&(~Q(data__Артикул__exact = ''))).values('id','data')
    else:
        # artikules = AluProfilesData.objects.filter((~Q(data__Артикул__icontains = '0'))&(~Q(data__Артикул__icontains = 0)))[:50].values('id','data')
        artikules = AluProfilesData.objects.filter((~Q(data__Артикул__exact = '0'))&(~Q(data__Артикул__exact = 0))&(~Q(data__Артикул__exact = ' '))&(~Q(data__Артикул__exact = '')))[:50].values('id','data')
